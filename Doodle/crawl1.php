<?php
$start = "https://www.razer.com/";
$pdo = new PDO('mysql:host=127.0.0.1;dbname=doodle1','root','');
$already_crawled = array();
$crawling = array();
function get_details($url) {
    $options = array('http'=>array('method'=>"GET", 'headers'=>"User-Agent: kewfBot/0.1\n"));
...
function follow_links($url) {
    ...
    foreach ($linklist as $link) {
        ...
        if (!in_array($l, $already_crawled)) {
                ...
                $rows = $pdo->query("SELECT * FROM sites WHERE url_hash='".md5($details->URL)."'");
